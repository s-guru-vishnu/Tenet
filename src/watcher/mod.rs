...
pub async fn watch_directory(dir: &Path) -> Result<()> {
    ...
    let initial_count =
        versioning::create_initial_snapshot(&watched_dir, &tenet_dir, &mut metadata, &ignore_rules)?;
    println!(
        "{}",
    ...
    loop {
        match rx.recv() {
            Ok(Ok(events)) => {
                ...
                // Process the batch
                match processor::process_events(&paths, watched_dir, tenet_dir, metadata, ignore_rules)
                {
                    Ok(count) => {
                        if count > 0 {
                            println!(
                                ...
                                if !ignore_rules.should_ignore(path, watched_dir) {
                                    if let Ok(rel) = utils::relative_path(path, watched_dir) {
                                        if path.exists() {
                                            ...
                                            println!("   {} {} {}", "✗".red(), rel, "(deleted)".bright_black());
                                        }
                                    }
                                }
                            }
                    ...
                    Err(e) => {
                        eprintln!(
                            "{}",
                            format!("⚠️  Error processing events: {}", e).red()
                        );
                    }
                }
            }
