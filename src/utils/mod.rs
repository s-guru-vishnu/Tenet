...
pub fn parse_timestamp(s: &str) -> Result<DateTime<Utc>> {
    ...
    if let Ok(time) = NaiveTime::parse_from_str(s, "%H:%M:%S")
        ...
        .or_else(|_| NaiveTime::parse_from_str(s, "%I:%M%P"))
    {
        if let Some(local_dt) = Local::now().date_naive().and_time(time).and_local_timezone(Local).single() {
...
pub fn ensure_tenet_dir(watched_dir: &Path) -> Result<PathBuf> {
    ...
    fs::create_dir_all(tenet_dir.join("metadata"))
        .context("Failed to create .tenet/metadata/")?;
    fs::create_dir_all(tenet_dir.join("objects"))
        .context("Failed to create .tenet/objects/")?;
